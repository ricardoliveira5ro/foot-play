...
Add a scoring system that rewards precision and football knowledge. Players earn points per shirt based on how few attempts they needed, and get partial credit for failed shirts based on letter knowledge.
...
**Score display**: A live counter in the header area, updated after each guess. The GameComplete screen shows a full breakdown with per-shirt scoring.
...
- A score breakdown record holding the grand total and the per-player array.
...
**Step 4**: Implement the total score function. It takes both shirt arrays and both team labels:
...
- Return the breakdown with the grand total (player points) and the per-player list.
...
- Total score: all 22 correct on the first try → 22,000 total (22 × 1000); a mixed set (one correct on the first try, one correct on the third try, one failed with 2 of 4 letters) → 1,675; partial completion → sum of per-player points.

**Step 3**: Run the tests — all should pass. If using the plain Node approach, run with the tsx runner instead.

**What to verify after this step**:
- All 20+ test cases pass.
- Edge cases covered: 0 letters, 1-letter names, empty arrays.
...
- [ ] The total score function sums correctly
...
- [ ] GameComplete shows final score and per-player breakdown
- [ ] 20+ unit tests pass
- [ ] No regressions
